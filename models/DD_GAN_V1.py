...
import torch.nn as nn
import torch 
from dataset_128 import Res_13Shapefea_ZS,Res_13fea_ZS_triplet_semgen
...
class DD_GAN_V1(object):
    ...
    def build_model(self):
        ...
        self.disc_ske = Discriminator(in_dim=2048, noise=True, use_batchnorm=False).cuda()
        # Image discriminator
        self.disc_img = Discriminator(in_dim=2048, noise=True, use_batchnorm=False).cuda()
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.epoch+1):
            for idx, datas in enumerate(self.sketch_dataloader, 0):
                if idx+1 > 80: continue
                sketch_oral,shape_oral,shape_neg_oral,sketch_label,sketch_w2v,shape_neg_w2v = datas
                # the dataset reture datas: sketch_feature,shape_feature,shape_negative_feature,\
                #sketch_label,sketch_word_embedding_vector,shape_negative_word_embedding_vector.

                # print(sketch_oral.shape,shape_oral.shape,shape_neg_oral.shape,sketch_label.shape,sketch_w2v.shape,shape_neg_w2v.shape)
                #torch.Size([256, 2048]) torch.Size([256, 2048]) torch.Size([256, 2048]) torch.Size([256]) torch.Size([256, 300]) torch.Size([256, 300])
                ...
                sketch_fea_con_1,sketch_con_cls = self.gen_skecon(sketch_oral_1)
                sketch_sty, _ = self.gen_skesty(sketch_oral_1)

                # extract sketch content and style feature
                shape_fea_con_1,shape_con_cls = self.gen_imgcon(shape_oral_1)
                image_sty, _ = self.gen_imgsty(shape_oral_1)
                shape_fea_con_neg_1,shape_con_neg_cls = self.gen_imgcon(shape_neg_oral_1)
                image_neg_sty, _ = self.gen_imgsty(shape_neg_oral_1)

                # map the visual feature to semantic space
                sketch_sem = self.semantic_sk(sketch_fea_con_1)
                shape_sem = self.semantic_im(shape_fea_con_1)
                shape_neg_sem = self.semantic_im(shape_fea_con_neg_1)
                ...
                img2ske = self.skedec(
                    torch.cat((shape_fea_con_1, sketch_sty), 1))
                ...
                cross_loss_1 = self.contrastiveLoss(sketch_fea_con_1,shape_fea_con_1,shape_fea_con_neg_1, 20)
                ...
                self.loss = cross_loss_1 + cls_loss + loss_sem + loss_gen_adv
                
                self.optimizer_gen.zero_grad()
                self.loss.backward()
                self.optimizer_gen.step()

                print("Epoch: [%2d] [%4d/%4d] time: %2dm %2ds loss_g: %.4f loss_d: %.4f acc1: %.4f acc2: %.4f"  \
                ...
                self.log_string("Epoch: [%2d] [%4d/%4d] time: %2dm %2ds loss_g: %.4f loss_d: %.4f acc1: %.4f acc2: %.4f" \
                          % (epoch, idx+1, self.num_batches, (time.time()-start_time)/60,(time.time()-start_time)%60,self.loss.item(),loss_D.item(),acc_sketch1,acc_shape1))
            if epoch % 1 == 0:
    ...
    def test(self):
        ...
        self.gen_skesty.eval()
        self.gen_imgsty.eval()
        sketchLabMatrix = []
        shapeLabMatrix = []
        sketchMatrix = []
        shapeMatrix = []
        ...
        sketch_dataloader = torch.utils.data.DataLoader(sketch_dataset,batch_size = 4,shuffle=False, num_workers= 4)
        shape_proj_dataset = Res_13Shapefea_ZS()
        shape_dataloader = torch.utils.data.DataLoader(shape_proj_dataset,batch_size = 4,shuffle = False,num_workers = 4)
        for idx,datas in enumerate(sketch_dataloader,0):
            sketch_oral,sketch_label = datas
        ...
        for idx, datas2 in enumerate(shape_dataloader, 0):
            shape_oral,shape_label = datas2
            # datas2: shape_feature,shape_label
    ...
    def save_h5(self, data, h5_filename):
        import h5py
    ...
    def load(self, checkpoint_dir):
        ...
        # ----------------- load D -------------------
        if not self.pretrain_model is None:
            resume_file_D = os.path.join(
                checkpoint_dir, self.pretrain_model)
            flag_D = os.path.isfile(resume_file_D)
            if flag_D == False:
                print('DD_GAN--> Error: no checkpoint directory found!')
                exit()
            else:
                print('resume_file_DD_GAN------>: {}'.format(resume_file_D))
                ...
                # self.disc_ske.load_state_dict(checkpoint['disc_ske'])
                # self.disc_img.load_state_dict(checkpoint['disc_img'])
                D_epoch = checkpoint['gen_epoch']
        else:
            print(" [*] Failed to find the pretrain_model")
            exit()

        print(" [*] Success to load model --> {}".format(self.pretrain_model))
        return True, D_epoch
...
class Generator_content(nn.Module):
    def __init__(self, in_dim=2048, out_dim=300, noise=True, use_batchnorm=False, use_dropout=False):
...
class Generator(nn.Module):
    def __init__(self, in_dim=512, out_dim=300, noise=True, use_batchnorm=False, use_dropout=False):
...
class Discriminator(nn.Module):
    def __init__(self, in_dim=300, out_dim=1, noise=True, use_batchnorm=False, use_dropout=False,
                 use_sigmoid=False):
